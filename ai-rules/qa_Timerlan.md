...
3. **Walkthrough videos** of the suite running. Two MP4s currently ship: `qa/walkthrough.mp4` (purchase flow) and `qa/walkthrough-ai.mp4` (AI chatbot). Generated by running `QA_VIDEO=on npm run test:e2e`, then concatenating the per-spec webms with ffmpeg + slowing 2× + transcoding to H.264 MP4.

Scope to date:
- **Iteration 1 — Purchase flow:** home → menu → cart → auth → order. Six specs in `qa/e2e/flows/{home,menu-cart,auth,order}.spec.ts`. Green.
- **Iteration 2 — AI chatbot:** `/ai-assistant` page → POST `/api/v1/ai/chatbot/chat` → FastAPI + LangChain agent on `:8002` → tool calls into the backend. Four specs in `qa/e2e/flows/ai-chat.spec.ts`. Green.
...
## Iteration 2 — AI chatbot (added 2026-05-12)

### Why this iteration is different from the purchase flow

- The AI page hits a real LLM (**OpenAI gpt-4o** via LangChain). Replies are **non-deterministic** and each call costs money — so the suite is small (4 tests) and assertions are deliberately lenient.
- The chatbot sits behind a proxy chain: browser → `POST /api/v1/ai/chatbot/chat` (Express, requires `SERVICE_KEY` env) → FastAPI on `:8002` (verifies `X-Service-Key`) → LangChain agent with 7 tools (`search_menu`, `get_restaurant_info`, `get_restaurants`, `find_restaurant`, `add_to_cart`, `view_cart`, `place_order`) → backend API (when tools call out).
- Session state lives in `localStorage["ai-chat-session-id"]` (UUID). The chatbot service keeps an in-memory `{messages, cart, memory}` per session, TTL 30 min.

### Data flow (one AI chat message)

```
Browser /ai-assistant
  └── POST /api/v1/ai/chatbot/chat   { sessionId, restaurantId?, message }
        ↓ (Express ai.controller forwards X-Service-Key + optional Bearer JWT)
      FastAPI :8002 /chat
        ↓ require_service_key
      LangChain AgentExecutor (gpt-4o, conversation memory)
        ↓ tool calls (when the model decides)
      Backend API (search/menu/restaurants/orders)
        ↓
      reply text + updated session cart
  ← JSON { reply, cartUpdated, cart, sessionId }
  React updates the messages list and AI Cart sidebar
```

### Procedure additions

1. **Probe before scaffolding** — `curl http://localhost:8002/health` and a one-shot `curl -X POST .../ai/chatbot/chat -d '{"sessionId":"probe","restaurantId":"rest_001","message":"hello"}'`. Confirms OPENAI_API_KEY + SERVICE_KEY are wired and the LLM is reachable. Saves ~30 s of failed-test debugging if the backend or AI service isn't healthy.
2. **Brief the agent on cost + nondeterminism** — keep tests ≤ 4, ≥ 30 s timeout on each chat round-trip, lenient assertions (JSON shape, regex partials, never exact text).
3. **Anchor assertions on the JSON, not the rendered text** — `page.waitForResponse(...)` then `response.json()` gives a deterministic snapshot of `{reply, cart, cartUpdated}`. Use the rendered DOM only to verify wiring (e.g. AI cart sidebar reflects `cart` updates).
4. **Preset-button test is a freebie** — clicking a "Try asking" suggestion populates the input via plain React state; no LLM call, no flakes, fast UI coverage.
5. **The walkthrough video for this iteration is separate** — `qa/walkthrough-ai.mp4` (21.9 s, 231 KB). Same ffmpeg recipe as iteration 1 (`setpts=2.0*PTS`, libx264, yuv420p, faststart).

### Things that bit me on iteration 2

- **`xpath=ancestor::div[1]` resolves to the *closest* ancestor div**, which on the AI page was the inner heading wrapper, not the outer card. The `<p>` we wanted is a *sibling* of that wrapper, not a descendant. Fix: `ancestor::div[contains(@class,"flex")]/following-sibling::p[1]`. Lesson: scope XPath by class predicate, not just by axis position.
- **OpenAI sometimes "decides" not to call the tool** the prompt asks for. The first AI-03 draft asserted the reply text contained "added to cart" — which is not how the agent always phrases it. The fix was to read the response JSON and assert `cart.length >= 1 && cart.some(i => i.menuItemId === "item_001")` instead.

## Where things live

| Thing | Location |
|---|---|
| Playwright config | `qa/playwright.config.ts` (honours `QA_VIDEO=on`) |
| Page Object Models | `qa/e2e/pages/` (incl. `AiAssistantPage.ts`) |
| Specs | `qa/e2e/flows/` (incl. `ai-chat.spec.ts`) |
...
| Walkthrough — purchase flow | `qa/walkthrough.mp4` |
| Walkthrough — AI chatbot | `qa/walkthrough-ai.mp4` |
| Broader QA strategy | `qa/PLAN.md` |
| This procedure | `ai-rules/qa_Timerlan.md` |
| Public-facing report | Notion page **[QA — E2E Testing](https://www.notion.so/QA-E2E-Testing-home-menu-cart-auth-order-35e4aac848fc813296b0f3c40e9f0ba0)** under *Online restaurant* |
